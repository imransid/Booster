  ...
  try {
    ...
    if (data !== null) {
      ...
      if (data_load.length != undefined) {
        ...
        data_load.wallet_id == walletIdNo
          ? (wallet_detaits_result = data_load)
          : null;
      }
  ...
  try {
    ...
    if (data !== null) {
      ...
      if (data_load.length != undefined) {
        ...
        output_result = output_result.filter(e => e != null);
      } else {
        data_load.walletId == walletIdNo
          ? (output_result = data_load)
          : (output_result = null);
      }
...
export const all_Transection = function*(action) {
  ...
  const retive_wallet_details = yield call(
    retrieveWalletDetails,
    action.walletId
  );
  yield put({
    type: actionType.TRANSECTION_RESULT,
    result: retive_data,
    result_wallet: retive_data_wallet,
    wallet_detaits: retive_wallet_details,
    wallet_id: action.walletId
  });
};
  ...
  try {
    ...
    AsyncStorage.getItem("wallet@Card").then(res => {
      ...
      if (data.length == undefined) {
        CType == "Bill"
          ? (avamount = parseInt(data.avalible_balance) + parseInt(amounT))
          : (avamount = parseInt(data.avalible_balance) - parseInt(amounT));
        if (
          CType == "Bill" &&
          Card_Type == "DABIT" &&
          avamount > parseInt(data.balance)
        ) {
        ...
        CType == "Bill"
          ? (avamount =
              parseInt(data[objIndex].avalible_balance) + parseInt(amounT))
          : (avamount =
              parseInt(data[objIndex].avalible_balance) - parseInt(amounT));
        ...
        if (
          CType == "Bill" &&
          Card_Type == "DABIT" &&
          avamount > parseInt(data[objIndex].balance)
        ) {
          UpdatedNewBalance = avamount;
        } else {
    ...
    AsyncStorage.getItem("transection@Data").then(res => {
      ...
      if (data.length == undefined) {
        ...
        let objIndex = data.findIndex(
          obj => obj.transectionid === props.data.transectionid
        );
      ...
      if (update_walletId) {
        AsyncStorage.getItem("wallet@Card").then(res => {
          ...
          if (data.length == undefined) {
            result = data;
            result.avalible_balance =
              parseInt(data.avalible_balance) + parseInt(props.data.value);
          } else {
            let objIndex = data.findIndex(
              obj => obj.wallet_id === update_walletId
            );
            let updateObj = {
              ...data[objIndex],
              avalible_balance:
                parseInt(data[objIndex].avalible_balance) +
      ...
      if (result) {
        ...
        console.log(
          "no data in our result check: saga_actions/All_Data_Transections"
        );
      }
    });
  } catch (error) {
...
export const addTransections = function*(action) {
  yield call(
    UpdateBalance,
    action.result.walletId,
    action.result.amount,
    action.category,
    action.Card_Type
  );
